...
function App() {
  ...
  const watchAdForBoost = (boostType) => {
    if (window.Adsgram) {
      ...
      AdController.show()
        .then(async () => {
          // Якщо Adsgram спрацював — шлемо запит на сервер
          triggerNotification('success');
          try {
        ...
        .catch(async () => {
          // 🔥 ЯКЩО ADSGRAM НЕМАЄ РЕКЛАМИ — ВІДКРИВАЄМО ТВІЙ DIRECT LINK 🔥
          console.log("Adsgram empty, switching to Monetag Direct Link...");
          
          // Твоє посилання від Monetag
          const monetagDirectLink = "https://omg10.com/4/10832366";
          
          // Відкриваємо рекламу в новій вкладці
          window.open(monetagDirectLink, '_blank');

          // Одразу зараховуємо буст (оскільки Direct Link не має callback-завершення)
          triggerNotification('success');
          try {
            ...
            tg.showAlert("Резервний буст активовано через Monetag! 🚀");
          } catch (err) { 
            tg.showAlert("Помилка активації. Спробуй ще раз."); 
          }
        });
    } else {
  ...
  const buyUpgrade = async (item, currentCost) => {
    ...
    } catch (err) { tg.showAlert(err.response?.data?.error || "Помилка покупки"); }
  };
  ...
  const submitSquad = async () => {
    ...
    try {
      ...
      setUserData(res.data.user); triggerNotification('success'); setShowSquadModal(false); tg.showAlert(`Успіх! Ти приєднався до скваду @${res.data.squad.username}`);
  ...
  const claimAchievement = async (id, reward, goal, type = 'points') => {
    ...
    if (type === 'refs' && (userData.referrals_count || 0) < goal) { tg.showAlert(`Тобі потрібно ${goal} АКТИВНИХ друзів!`); return; }
    ...
    tg.showConfirm("АДМІН! Завершити сезон прямо зараз?", async (agreed) => {
      if (agreed) {
        try {
          ...
          tg.showAlert("✅ СЕЗОН ЗАВЕРШЕНО! Перевір бот."); window.location.reload();
        } catch (err) { tg.showAlert("Помилка Адмінки."); }
      }
    });
  };
        ...
        <div className="flex flex-col items-center justify-center bg-gray-900 border border-gray-700 w-full max-w-sm rounded-3xl p-8 space-y-6 shadow-2xl animate-fade-in mb-10">
          ...
          {onboardingStep === 0 && (
            <>
              ...
              <p className="text-gray-300 text-sm leading-relaxed px-2">Це не просто клікер. Це чесна гра, де ми віддаємо <span className="font-bold text-white">20% доходу</span> гравцям та сквадам наприкінці сезону.</p>
            </>
          )}
          {onboardingStep === 1 && (
            <>
          ...
          {onboardingStep === 2 && (
            <>
              ...
              <p className="text-gray-300 text-sm leading-relaxed px-2">Зароблені монети витрачай на бізнеси. Качка буде працювати і приносити гроші, навіть коли ти офлайн (до 3-х годин).</p>
            </>
          )}
          {onboardingStep === 3 && (
            <>
              ...
              <p className="text-gray-300 text-sm leading-relaxed px-2">Об'єднуйся в команди з друзями! Якщо твій сквад переможе, всі його учасники отримають величезний бонус.</p>
            </>
          )}

        </div>
        <button onClick={() => { triggerHaptic('light'); if (onboardingStep < 3) setOnboardingStep(prev => prev + 1); else finishOnboarding(); }} className="bg-yellow-500 text-gray-900 font-black text-lg py-4 w-[90%] rounded-2xl active:scale-95 absolute bottom-8 shadow-xl">
    ...
    <div className={`flex flex-col h-screen bg-gradient-to-b ${bgColors[Math.min(level-1, 9)]} select-none overflow-hidden text-white transition-colors duration-1000`}>
          ...
          <div className="flex-1 flex flex-col px-4 animate-fade-in">
            ...
            <div className="w-full bg-gray-800/90 backdrop-blur-md p-4 rounded-3xl border border-gray-700/50 shrink-0 shadow-xl">
              <div className="flex justify-between items-center mb-1">
                <span className="text-xs font-bold text-blue-300">⚡ Енергія</span><span className="text-xs font-bold text-blue-300">{Math.floor(energy)} / {MAX_ENERGY}</span>
              </div>
            ...
            {shopSubTab === 'business' ? (
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className={`bg-gray-800 border p-4 rounded-3xl text-center flex flex-col items-center justify-between h-48 ${userData.current_skin === 'default' ? 'border-yellow-400 shadow-[0_0_15px_rgba(234,179,8,0.2)]' : 'border-gray-700'}`}>
                  ...
                  <button onClick={resetToEvolutionSkin} className={`w-full py-2 rounded-xl text-xs font-bold active:scale-95 transition-all ${userData.current_skin === 'default' ? 'bg-yellow-400 text-gray-900' : 'bg-gray-600 text-white'}`}>{userData.current_skin === 'default' ? 'Одягнено' : 'Одягнути'}</button>
                </div>
                {SKINS.map(skin => {
                  const isOwned = userData.unlocked_skins?.includes(skin.id);
          ...
          <div className="flex-1 flex flex-col p-4 animate-fade-in overflow-y-auto">
            ...
            <div className="grid grid-cols-2 gap-3 mb-6">
              <div className="bg-gray-800 border border-gray-700 p-3 rounded-2xl flex flex-col items-center text-center gap-2">
                ...
                <button disabled={userData.ad_energy_left <= 0 || isCooldown(userData.ad_energy_ready_at)} onClick={() => watchAdForBoost('energy')} className={`w-full text-white text-[10px] font-bold py-2 rounded-lg ${userData.ad_energy_left <= 0 ? 'bg-gray-700 text-gray-500' : isCooldown(userData.ad_energy_ready_at) ? 'bg-gray-600 text-orange-300' : 'bg-blue-500 active:scale-95'}`}>{userData.ad_energy_left <= 0 ? 'Завтра' : isCooldown(userData.ad_energy_ready_at) ? `⏳ ${getRemainingMin(userData.ad_energy_ready_at)} хв` : 'Дивитись'}</button>
              </div>
              <div className="bg-gray-800 border border-gray-700 p-3 rounded-2xl flex flex-col items-center text-center gap-2">
                <div className="text-2xl">🚀</div><h3 className="font-bold text-xs text-white">Буст x5</h3><p className="text-[10px] text-gray-400">{userData.ad_x5_left}/3</p>
                <button disabled={userData.ad_x5_left <= 0 || isCooldown(userData.ad_x5_ready_at)} onClick={() => watchAdForBoost('x5')} className={`w-full text-white text-[10px] font-bold py-2 rounded-lg ${userData.ad_x5_left <= 0 ? 'bg-gray-700 text-gray-500' : isCooldown(userData.ad_x5_ready_at) ? 'bg-gray-600 text-orange-300' : 'bg-orange-500 active:scale-95'}`}>{userData.ad_x5_left <= 0 ? 'Завтра' : isCooldown(userData.ad_x5_ready_at) ? `⏳ ${getRemainingMin(userData.ad_x5_ready_at)} хв` : 'Дивитись'}</button>
              </div>
              <div className="bg-gray-800 border border-gray-700 p-3 rounded-2xl flex flex-col items-center text-center gap-2">
                <div className="text-2xl">🤖</div><h3 className="font-bold text-xs text-white">Автоклікер</h3><p className="text-[10px] text-gray-400">{userData.ad_autoclick_left}/3</p>
        ...
        {activeTab === 'info' && (
          <div className="flex-1 flex flex-col p-4 animate-fade-in overflow-y-auto">
            ...
            <div className="bg-gray-800 border border-gray-700 p-5 rounded-3xl text-sm text-gray-300 space-y-4 mb-6 shadow-xl">
              <p><strong className="text-yellow-400 text-base">🦆 Мета гри:</strong> Клікай, заробляй монети, купуй бізнеси та піднімайся в лігах. Чим вищий рівень — тим більший заробіток!</p>
              
              <hr className="border-gray-700"/>
              
              <p><strong className="text-green-400 text-base">💸 Жодних Донатів:</strong> Найголовніший донат — це ваш час! Ми не продаємо нічого за реальні гроші.</p>
              
              <hr className="border-gray-700"/>
              
              <p><strong className="text-white text-base">🏆 Чесний рейтинг:</strong> Кожного 1-го числа сезону ми фіксуємо найкращих гравців та обнуляємо монети (але ваші скіни та запрошені друзі залишаються з вами назавжди).</p>
              
              <hr className="border-gray-700"/>
              
              <p>
                <strong className="text-orange-400 text-base">💰 Призовий фонд (20% від доходу):</strong><br/>
                Ми віддаємо рівно <b>20%</b> від усього доходу з реклами щомісяця!<br/><br/>
                • <b>10%</b> — ділять між собою ТОП-11 найкращих гравців сезону.<br/>
                • <b>10%</b> — забирає ТОП-1 Сквад (половину забирає творець скваду, а іншу половину ділять ТОП-5 найактивніших учасників).
              </p>
            </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 h-[80px] bg-gray-900/95 backdrop-blur-lg border-t border-gray-800 grid grid-cols-5 px-1 pb-safe z-40">
        <button onClick={() => { triggerSelection(); setActiveTab('tap'); }} className={`flex flex-col items-center justify-center transition-colors ${activeTab === 'tap' ? 'text-yellow-400' : 'text-gray-500'}`}>
          <span className="text-2xl mb-1">🦆</span><span className="text-[9px] font-bold uppercase">Грай</span>
        </button>
        <button onClick={() => { triggerSelection(); setActiveTab('shop'); }} className={`flex flex-col items-center justify-center transition-colors ${activeTab === 'shop' ? 'text-yellow-400' : 'text-gray-500'}`}>
          <span className="text-2xl mb-1">🛒</span><span className="text-[9px] font-bold uppercase">Магазин</span>
        </button>
        <button onClick={() => { triggerSelection(); setActiveTab('tasks'); }} className={`flex flex-col items-center justify-center transition-colors ${activeTab === 'tasks' ? 'text-yellow-400' : 'text-gray-500'}`}>
          <span className="text-2xl mb-1">🎯</span><span className="text-[9px] font-bold uppercase">Завдання</span>
        </button>
        <button onClick={() => { triggerSelection(); setActiveTab('friends'); }} className={`flex flex-col items-center justify-center transition-colors ${activeTab === 'friends' ? 'text-yellow-400' : 'text-gray-500'}`}>
          <span className="text-2xl mb-1">👥</span><span className="text-[9px] font-bold uppercase">Друзі</span>
        </button>
        <button onClick={() => { triggerSelection(); setActiveTab('info'); }} className={`flex flex-col items-center justify-center transition-colors ${activeTab === 'info' ? 'text-yellow-400' : 'text-gray-500'}`}>
          <span className="text-2xl mb-1">ℹ️</span><span className="text-[9px] font-bold uppercase">Інфо</span>
        </button>
      </div>
      ...
      {showLeaderboard && (
        <div className="absolute inset-0 z-[80] bg-gray-950/95 flex flex-col p-4 animate-fade-in">
          ...
          <div className="flex bg-gray-800 rounded-xl p-1 mb-4 shrink-0">
            <button onClick={() => setLeaderboardTab('players')} className={`flex-1 py-2 font-bold rounded-lg transition-all ${leaderboardTab === 'players' ? 'bg-gray-700 text-white shadow' : 'text-gray-500'}`}>Гравці</button>
            <button onClick={() => setLeaderboardTab('squads')} className={`flex-1 py-2 font-bold rounded-lg transition-all ${leaderboardTab === 'squads' ? 'bg-gray-700 text-white shadow' : 'text-gray-500'}`}>Сквади</button>
          </div>
      ...
      {showSquadModal && (
        <div className="absolute inset-0 z-[90] bg-gray-950/95 flex flex-col p-6 items-center justify-center animate-fade-in">
          <div className="bg-gray-900 border border-gray-700 rounded-3xl p-6 w-full max-w-sm shadow-2xl relative">
            ...
            <div className="text-6xl text-center mb-4">🛡️</div><h2 className="text-2xl font-black text-white mb-2 text-center">Вступити в Сквад</h2><p className="text-xs text-gray-400 text-center mb-6">Введи @username скваду або створи новий, просто ввівши назву.</p>
            ...
            <button onClick={submitSquad} className="bg-blue-600 text-white font-bold py-3 rounded-xl w-full active:scale-95 transition-all">Приєднатися</button>
          </div>
        </div>
      )}
